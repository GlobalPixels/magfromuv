...
public:
  ...
  long numRows;
  BoundingBox extent;
  double cellSize, cellSizeX, cellSizeY;
...
//            && (extent.left == testGrid->extent.left) && (extent.bottom == testGrid->extent.bottom)
 //           && (cellSize == testGrid->cellSize));
  ...
  bool GetGridLoc(float lon, float lat, GridLoc *pt) {
    ...
    float xLoc = xDiff/cellSizeX;
    float yLoc = yDiff/cellSizeY;
    pt->x = (long)xLoc;
    pt->y = (long)yLoc;
    ...
    if ((extent.left - cellSizeX) > lon || (extent.right + cellSizeX) < lon || (extent.bottom - cellSizeX) > lat || (extent.top + cellSizeX) < lat) {
  ...
  bool GetRefLoc(long x, long y, RefLoc *pt) {
    pt->x = (float)x * cellSizeX + extent.left;
    pt->y = extent.top - (float)y * cellSizeY;
    return true;
  }
